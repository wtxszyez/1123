--[[--
----------------------------------------------------------------------------
Open 360 Video Metadata Tool v4.0.1 for Fusion - 2018-12-31
by Andrew Hazelden
...
Step 1. Start Fusion and open a new comp. Then run the "Script > KartaVR > Send Media to > Open 360 Video Metadata Tool" menu item to launch the YouTube 360 metadata program.

Installing the Metadata Tool:

You can use the Reactor package "Bin/Spatial Media Metadata Injector" to add this tool to your system.


Downloading the Metadata Tool Manually:

You could also download and install the tool manually. You would need to edit the "viewerProgram" variable in the "spatialMediaMetadataTool()" function below.
...
On Mac OS X the metadata tool has to be copied to:
...
function spatialMediaMetadataTool()
	-- Viewer Variables
	viewerProgram = nil
	command = nil
	...
	if platform == 'Windows' then
		-- Running on Windows
		viewerProgram = '"' .. app:MapPath('Reactor:/Deploy/Bin/360VideosMetadata/bin/Spatial Media Metadata Injector.exe') .. '"' 
		-- viewerProgram = '"C:\\Program Files\\Spatial Media Metadata Injector.exe"'
		-- viewerProgram = '"C:\\Program Files (x86)\\Spatial Media Metadata Injector.exe"'
		command = 'start "" ' .. viewerProgram
	...
	elseif platform == 'Mac' then
		...
		viewerProgram = '"' .. app:MapPath('Reactor:/Deploy/Bin/360VideosMetadata/bin/') .. 'Spatial Media Metadata Injector.app' .. '"'
		-- viewerProgram = '"/Applications/Spatial Media Metadata Injector.app"'
	...
	elseif platform == 'Linux' then
		-- Running on Linux
		print('Spatial Media Metadata Injector is not available for Linux yet.')
	else
