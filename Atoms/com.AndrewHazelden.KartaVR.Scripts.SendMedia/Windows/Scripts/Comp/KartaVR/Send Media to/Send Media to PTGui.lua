--[[--
----------------------------------------------------------------------------
Send Media to PTGui v4.0 for Fusion - 2018-12-11
by Andrew Hazelden
...
The Send Media to PTGui script is a module from [KartaVR](http://www.andrewhazelden.com/blog/downloads/kartavr/) that will send all of the selected loader node files to the PTgui Stitching program. 

How to use the Script:

Step 1. Start Fusion and open a new comp. Select a set of loader and saver nodes in the flow view. Then run the "Script > KartaVR > Send Media to > Send Media to PTGui" menu item to load the media in PTGui.
...
function ptguiStitcher()
	...
	if platform == 'Windows' then
		...
		command = 'start "" ' .. comp:MapPath(getPreferenceData('KartaVR.SendMedia.PTGuiFile', defaultViewerProgram, printStatus)) .. mediaList
	...
	elseif platform == 'Mac' then
		...
		command = 'open -a ' .. string.gsub(comp:MapPath(getPreferenceData('KartaVR.SendMedia.PTGuiFile', defaultViewerProgram, printStatus)), '[/]$', '') .. ' --args ' .. mediaList
...
print ('Send Loaders to PTGui is running on ' .. platform .. ' with Fusion ' .. eyeon._VERSION)
