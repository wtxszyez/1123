--[[--
----------------------------------------------------------------------------
Send Frame to Corel Photo Paint v4.0 for Fusion - 2018-12-11
by Andrew Hazelden
...
The Send Frame to Corel Photo Paint script is a module from [KartaVR](http://www.andrewhazelden.com/blog/downloads/kartavr/) that will send your currently selected file loader or saver node files to Corel Photo Paint. 

How to use the Script:

Step 1. Start Fusion and open a new comp. Select and activate a node in the flow view. Then run the "Script > KartaVR > Send Media to > Send Frame to Corel Photo Paint" menu item to load the media in Corel Photo Paint.

Note: Corel Photo Paint writes the version number in the application filename like "C:\Program Files\Corel\CorelDRAW Graphics Suite X7\Programs64\CorelPP.exe". This means you will need to update the executable name in this script when you install a new version of Corel Photo Paint.
...
function corelLauncher(mediaFileName)
	-- Viewer Variables
	viewerProgram = nil
	command = nil
...
print ('Send Frame to Corel Photo Paint is running on ' .. platform .. ' with Fusion ' .. eyeon._VERSION)
...
if selectedNode then
	...
	elseif toolAttrs.TOOLS_RegID == 'Saver' then
		...
		imageFilename = 'dfm_corel_' .. selectedNode.Name .. '.' .. viewportSnapshotImageFormat
	...
	print('[Send Frame to Corel Photo Paint] No media node was selected. Please select and activate a loader or saver node in the flow view.')
	err = true
end
