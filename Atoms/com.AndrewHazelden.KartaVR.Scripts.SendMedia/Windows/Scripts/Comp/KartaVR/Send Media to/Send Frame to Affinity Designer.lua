--[[--
----------------------------------------------------------------------------
Send Frame to Affinity Designer v4.0.1 for Fusion - 2018-12-31
by Andrew Hazelden
...
The Send Frame to Affinity Designer script is a module from [KartaVR](http://www.andrewhazelden.com/blog/downloads/kartavr/) that will send your currently selected file loader or saver node files to Affinity Designer. 

How to use the Script:

Step 1. Start Fusion and open a new comp. Select and activate a node in the flow view. Then run the "Script > KartaVR > Send Media to > Send Frame to Affinity Designer" menu item to load the media in Affinity Designer.
...
function mediaViewerTool(mediaFileName)
	...
	affinityDesignerLauncher(mediaFileName)
end
...
function affinityDesignerLauncher(mediaFileName)
	-- Viewer Variables
	viewerProgram = nil
	command = nil
	...
	elseif platform == 'Mac' then
		-- Running on Mac
		defaultViewerProgram = '/Applications/Affinity Designer.app'
		viewerProgram = string.gsub(comp:MapPath(getPreferenceData('KartaVR.SendMedia.AffinityDesignerFile', defaultViewerProgram, printStatus)), '[/]$', '')
	...
	elseif platform == 'Linux' then
		-- Running on Linux
		print('Affinity Designer is not available for Linux yet.')
	else
...
print ('Send Frame to Affinity Designer is running on ' .. platform .. ' with Fusion ' .. eyeon._VERSION)
...
if selectedNode then
	...
	elseif toolAttrs.TOOLS_RegID == 'Saver' then
		...
		imageFilename = 'kvr_affinity_designer_' .. selectedNode.Name .. '.' .. viewportSnapshotImageFormat
	...
	print('[Send Frame to Affinity Designer] No media node was selected. Please select and activate a loader or saver node in the flow view.')
	err = true
end
