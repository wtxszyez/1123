--[[--
----------------------------------------------------------------------------
Send Frame to Affinity Photo v4.0.1 for Fusion - 2018-12-31
by Andrew Hazelden
...
The Send Frame to Affinity Photo script is a module from [KartaVR](http://www.andrewhazelden.com/blog/downloads/kartavr/) that will send your currently selected file loader or saver node files to Affinity Photo. 

How to use the Script:

Step 1. Start Fusion and open a new comp. Select and activate a node in the flow view. Then run the "Script > KartaVR > Send Media to > Send Frame to Affinity Photo" menu item to load the media in Affinity Photo.
...
function mediaViewerTool(mediaFileName)
	...
	affinityPhotoLauncher(mediaFileName)
end
...
function affinityPhotoLauncher(mediaFileName)
	-- Viewer Variables
	viewerProgram = nil
	command = nil
	...
	elseif platform == 'Mac' then
		-- Running on Mac
		defaultViewerProgram = '/Applications/Affinity Photo.app'
		
		viewerProgram = string.gsub(comp:MapPath(getPreferenceData('KartaVR.SendMedia.AffinityPhotoFile', defaultViewerProgram, printStatus)), '[/]$', '')
	...
	elseif platform == 'Linux' then
		-- Running on Linux
		print('Affinity Photo is not available for Linux yet.')
	else
...
print ('Send Frame to Affinity Photo is running on ' .. platform .. ' with Fusion ' .. eyeon._VERSION)
...
if selectedNode then
	...
	print('[Send Frame to Affinity Photo] No media node was selected. Please select and activate a loader or saver node in the flow view.')
	err = true
end
