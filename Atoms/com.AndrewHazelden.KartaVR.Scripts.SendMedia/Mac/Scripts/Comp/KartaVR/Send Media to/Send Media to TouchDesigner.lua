--[[--
----------------------------------------------------------------------------
Send Media to TouchDesigner v4.0.1 for Fusion - 2018-12-31
by Andrew Hazelden
...
The Send Media to TouchDesigner script is a module from [KartaVR](http://www.andrewhazelden.com/blog/downloads/kartavr/) that will send all of the selected loader node files to the TouchDesigner program. 

How to use the Script:

Step 1. Start Fusion and open a new comp. Select a set of loader and saver nodes in the flow view. Then run the "Script > KartaVR > Send Media to > Send Media to TouchDesigner" menu item to load the media in TouchDesigner.
...
function touchDesignerViewer()
	...
	if platform == 'Windows' then
		mediaList = string.gsub(mediaList, '\\', '/')
	end

	-- Open the Viewer tool
	if platform == 'Windows' then
		-- Running on Windows
		-- defaultViewerProgram = 'C:\\Program Files\\Derivative\\TouchDesigner088\\bin\\touchdesigner088.exe'
		
		defaultViewerProgram = 'C:\\Program Files\\Derivative\\TouchDesigner099\\bin\\touchdesigner099.exe'
		
		viewerProgram = getPreferenceData('KartaVR.SendMedia.TouchDesignerFile', defaultViewerProgram, printStatus)
	...
	elseif platform == 'Mac' then
		...
		defaultViewerProgram = '/Applications/TouchDesigner099.app'
		-- defaultViewerProgram = '/Applications/TouchDesigner099.app/Contents/MacOS/TouchDesigner099'
		viewerProgram = string.gsub(comp:MapPath(getPreferenceData('KartaVR.SendMedia.TouchDesignerFile', defaultViewerProgram, printStatus)), '[/]$', '')
	...
	elseif platform == 'Linux' then
		-- Running on Linux
		print('TouchDesigner is not available for Linux yet.')
	else
...
print ('Send Media to TouchDesigner is running on ' .. platform .. ' with Fusion ' .. eyeon._VERSION)
