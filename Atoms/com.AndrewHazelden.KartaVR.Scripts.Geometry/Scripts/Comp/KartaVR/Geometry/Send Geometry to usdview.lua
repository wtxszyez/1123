--[[--
----------------------------------------------------------------------------
Send Geometry to usdview - v4.2.1 2019-11-12
by Andrew Hazelden
...
The "Send Geometry to usdview" script takes AlembicMesh3D nodes that are selected in the Nodes view and sends the .abc models to usdview. The usdview program needs to be installed on the system and listed in the $PATH environment variable. usdview is primarily used to examine PIXAR OpenUSD (USDC/USDA) format scene graphs but it can also work with Alembic models too.

How to use the Script:

Step 1. Start Fusion and open a new comp. Select and activate a node in the flow view. 

Step 2. Run the "Script > KartaVR > Geometry > Send Geometry to usdview" menu item.
...
-- Example: openGeometry('Comp:/cube.usd', 'SurfaceFBXMesh', true)
function openGeometry(filename, nodeType, status)
	-- ------------------------------------
	-- Load the preferences
	-- ------------------------------------
	local defaultUsdviewFile = ''
	local command = ''
	
	if platform == 'Windows' then
		-- Reactor Install
		defaultUsdviewFile = comp:MapPath('usdview')
	elseif platform == 'Mac' then
		-- Take the trailing slash off the end of the final usdview path after the pathmap lookup
		defaultUsdviewFile = string.gsub(comp:MapPath('usdview'), '[/]$', '')
		-- defaultUsdviewFile = 'usdview'
	else
		defaultUsdviewFile = 'usdview'
	end
	...
	-- Location of usdview
	UsdviewFile = getPreferenceData('KartaVR.SendGeometry.UsdviewFile', defaultUsdviewFile, printStatus)
	soundEffect = getPreferenceData('KartaVR.SendGeometry.SoundEffect', 1, printStatus)
	
	msg = 'Customize the settings for sending geometry files to usdview. Please close usdview before running this tool.'
	...
	d[2] = {'UsdviewFile', Name = 'usdview Path', 'PathBrowse', Default = UsdviewFile}
	d[3] = {'SoundEffect', Name = 'Sound Effect', 'Dropdown', Default = soundEffect, Options = soundEffectList}
	
	dialog = comp:AskUser('Send Geometry to usdview', d)
	if dialog == nil then
		...
		if platform == 'Mac' then
			UsdviewFile = string.gsub(comp:MapPath(dialog.UsdviewFile), '[/]$', '')
		else
			UsdviewFile = comp:MapPath(dialog.UsdviewFile)
		end
		
		setPreferenceData('KartaVR.SendGeometry.UsdviewFile', UsdviewFile, printStatus)
			
		soundEffect = dialog.SoundEffect
		setPreferenceData('KartaVR.SendGeometry.SoundEffect', soundEffect, printStatus)
		
		print('[UsdviewFile] ' .. UsdviewFile)
		print('[SoundEffect] ' .. soundEffect)
	end
	
	-- Open the usdview tool
	if platform == 'Windows' then
		-- Running on Windows
		command = 'start "" "' .. UsdviewFile .. '" ' .. '"' .. filename .. '" '
		
		print('[Launch Command] ', command)
		os.execute(command)
	elseif platform == 'Mac' then
		-- Running on Mac
		command = 'open -a "' .. UsdviewFile .. '" --args ' .. '"' .. filename .. '" '
		
		print('[Launch Command] ', command)
		os.execute(command)
	elseif platform == 'Linux' then
		-- Running on Linux
		command = '"' .. UsdviewFile .. '" ' .. '"' .. filename .. '" '
		print('[Launch Command] ', command)
		os.execute(command)
	else
		print('[Platform] ', platform)
...
print ('Send Geometry to usdview is running on ' .. platform .. ' with Fusion ' .. eyeon._VERSION)
...
if selectedNode then
	...
	if nodeType == 'SurfaceAlembicMesh' then
	...
	print('[Send Geometry to usdview] No geometry node was selected. Please select and activate an AlembicMesh3D node in the flow view.')
	err = true
end
