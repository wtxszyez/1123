--[[--
----------------------------------------------------------------------------
Send Geometry to AC3D - v4.2 2019-11-05
by Andrew Hazelden
...
function openGeometry(filename, nodeType, status)
	...
	-- Location of AC3D
	AC3DFile = getPreferenceData('KartaVR.SendGeometry.AC3DFile', AC3DFile, printStatus)
	soundEffect = getPreferenceData('KartaVR.SendGeometry.SoundEffect', 1, printStatus)
	
	msg = 'Customize the settings for sending geometry files to AC3D. Please close AC3D before running this tool.'
	...
	d[2] = {'AC3DFile', Name = 'AC3D Path', 'PathBrowse', Default = AC3DFile}
	d[3] = {'SoundEffect', Name = 'Sound Effect', 'Dropdown', Default = soundEffect, Options = soundEffectList}
	
	dialog = comp:AskUser('Send Geometry to AC3D', d)
	if dialog == nil then
...
print ('Send Geometry to AC3D is running on ' .. platform .. ' with Fusion ' .. eyeon._VERSION)
...
if selectedNode then
	...
	print('[Send Geometry to AC3D] No geometry node was selected. Please select and activate a FBXMesh3D, ExporterFBX or AlembicMesh3D node in the flow view.')
	err = true
end
