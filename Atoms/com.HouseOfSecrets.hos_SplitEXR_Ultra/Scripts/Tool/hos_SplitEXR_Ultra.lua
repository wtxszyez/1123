...
function buildDialog()
	...
	dialog = {
		...
		{"Description", "Text", Lines = 3, Wrap = true, Default = "This script will split a multi-channel EXR image out into multiple Loader nodes.", ReadOnly = true, Width = 1.0},
...
function getLoader(verbose, dialogResult, tool)
	...
	for prefix, channels in pairs(channelData) do
		...
		-- Update the node position in the flow area
		flow:SetPos(loader, org_x_pos + (count * cdir), org_y_pos + (y_pos_add * count * (1 - cdir)))
	...
	-- Unlock the comp to allow Loader node file dialogs to be displayed
	comp:Unlock()
end
...
function main()
	...
	local verbose = dialogResult.verbose
	local splitAllSelectedNodes = dialogResult.splitAllSelectedNodes
	local cdir = dialogResult.cdir
	local skipAlpha = dialogResult.skipAlpha
	local tiles = dialogResult.tiles
	local grid = dialogResult.grid
