--[[
Action Listener v1.3 - 2017-11-21 07.15 AM
...
function ProcessAction(a, win)
  ...
  if retsTool ~= 'nil' then 
    retsToolName = tostring(ev.Rets.tool:GetAttrs('TOOLS_Name'))
  else
    retsToolName = 'nil'
  end
  ...
  if sender ~= 'nil' then 
    senderCompFilename =  tostring(ev.sender:GetAttrs('COMPS_FileName'))
  ...
  if itm.TranslateToLuaCheckbox.Checked then 
    ...
    elseif what == 'RunScript' then
      ...
      luaCommand = 'ldofile(comp:MapPath("' .. tostring(ev.Args.filename) .. '")) -- Run a script from disk'
